...
<?php
    ...
    if ( $mode == 'browse' ) {
        if ( sizeof( $imageList ) > 0 ) {
            ...
            $ogImg = $protocol . $_SERVER['HTTP_HOST'] . dirname($_SERVER["REQUEST_URI"]) . rawurlencode($imageList[0]);
            $ogUrl = $protocol . $_SERVER['HTTP_HOST'] . dirname($_SERVER["REQUEST_URI"]);
            $gotImg = true;
        } else {
    ...
    if ( $gotImg ) {
        $imageinfo = getimagesize($ogImg);
        $ogImgWidth = $imageinfo[0];
        $ogImgHeight = $imageinfo[1];
        $ogImgType = 'image/' . array( 'bmp', 'gif', 'jpg', 'png' )[$imageinfo[2]];
    }
?>
